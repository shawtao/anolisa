...
int trace_write_enter(struct syscall_trace_enter *ctx) 
{
    ...
    /*
     * Stdout/stderr capture stays tied to exec admission (child_pids only).
     * Do not widen to cgroup ∪ traced here: every write(1|2) in a cgroup would
     * flood the ringbuf (high-frequency syscall path).
     */
    if (!bpf_map_lookup_elem(&child_pids, &pid))
        return 0;
    u64 cg_id = get_cgroup_id_compat();
...
int trace_process_exit(void *ctx)
{
    ...
    /*
     * Exit events only for exec-tracked processes (child_pids). Matching stdout
     * gate avoids emitting cgroup-wide exits (scheduler hook fires for every task).
     */
    if (!bpf_map_lookup_elem(&child_pids, &pid))
        return 0;
    u64 cg_id = get_cgroup_id_compat();
